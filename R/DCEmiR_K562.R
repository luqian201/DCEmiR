...
## Data preprocessing ##
# Log transformation
miRNA_raw_norm <- t(miRNA_scRNA_raw)
mRNA_raw_norm <- t(mRNA_scRNA_raw)

# Remove genes with zero expression in more than 80% of cells
zero_ratio_miRNA <- rowSums(miRNA_raw_norm <= 9.998007e-05) / ncol(miRNA_raw_norm)
...
edge_counts <- sapply(DCEmiR_TargetScan_K562, function(net) {
