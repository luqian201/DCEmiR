...
dim(miRNA_filtered)
dim(mRNA_filtered)
miRNA_filtered <- Averg_Duplicate(miRNA_filtered)
mRNA_filtered <- Averg_Duplicate(mRNA_filtered)
dim(miRNA_filtered)
dim(mRNA_filtered)
# 合并并转置（行=基因，列=样本）
...
save(cellSpecificNets, file = " K562_116_11007_onlyDCEmiR_Null.RData")
